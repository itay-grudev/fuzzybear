...
import { search, type SearchElement } from '../src/fuzzybear'

/** The element shapes the shared examples are run against. */
type LabelledElement = string | { label: string, id: string }

function sharedExamples( term: string, matches: readonly LabelledElement[] ): void {
    ...
    it( 'returns results sorted in ascending distance', () => {
        const results = search( term, matches )
    ...
    it( 'is case insensitive by default', () => {
        const resultsLower = search( term.toLowerCase(), matches )
        const resultsUpper = search( term.toUpperCase(), matches )
        expect( resultsLower ).toEqual( resultsUpper )
    } )

    it( 'is case sensitive with caseSensitive set to true', () => {
        const resultsLower = search( term.toLowerCase(), matches, { caseSensitive: true } )
        const resultsUpper = search( term.toUpperCase(), matches, { caseSensitive: true } )
    ...
    it( 'returns 0 distances when the search term is empty', () => {
        ...
        ).toEqual( matches.map( () => 0 ) )
    } )
}

describe( 'search', () => {
    describe( 'with string elements', () => {
        const term = 'Identical'
        const matches = [ 'Identical', 'Identifier', 'dentical', 'Dental', 'dentist', 'different' ]
    ...
    it( 'raises an exception when no string values can be found', () => {
        ...
        expect(
            () => {
                // Elements that are neither strings nor labelled objects. Cast so that a
                // JavaScript caller's mistake can be exercised from TypeScript.
                search( 'term', [ 1, 2, 3 ] as unknown as SearchElement[] )
            },
        ).toThrow( 'Element without label is not searchable' )
    } )
} )
